      ...
      {isSelected && !isSubmitted  ? (
        <li
          ...
          <Star
            ...
            fillColor={selectQuestionsSkipped.includes(id) ? "fill-[#9ca8a7]" : "fill-[#14b8a6]"}
          />{" "}
          <span className="font-bold text-[16px] mr-3">Question {questionNumber}</span>{" "}
         {selectQuestionsSkipped.includes(id) && <span className="text-xs cursor-pointer hover:drop-shadow-xl font-semibold rounded-lg w-18 bg-orange-400 pb-1 px-2 text-white ">
            Skipped
          </span>}
        </li>
      ) : (
        <li
          ...
          <Star
            currentMode={currentMode}
            currentColor={currentColor}
            fillColor={selectAnsweredQuestions.includes(id) ? "fill-[#14b8a6]" : `fill-[#9ca8a7]`}
          />{" "}
