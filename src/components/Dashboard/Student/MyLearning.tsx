...
const MyLearning = () => {
  ...
  const transformedArray = studentProgress?.map(item => ({
    ...
    text: item.percentageProgress > 0 ? `${item.percentageProgress.toFixed(2)}%` : "0%"
}));
...
const distinctData = sortedData?.filter((item, index, self) => 
  index === self.findIndex((t) => (
      t.durationMillis === item.durationMillis
  ))
);
